...
class SearchCrawlController extends BaseController
{
    ...
    public function crawlSearch()
    {
        ...
        $baseUrl = 'https://pyony.com/search/';
        $page = 1;
        $totalInserted = 0;
        $totalUpdated = 0;
    ...
    private function parseEventData($html)
    {
        ...
        foreach ($items as $item) {
            $brand = trim($xpath->query('.//small[contains(@class, "font-")]', $item)->item(0)->nodeValue);
            $productName = trim($xpath->query('.//strong', $item)->item(0)->nodeValue);
            $eventType = trim($xpath->query('.//span[contains(@class, "badge bg-")]', $item)->item(0)->nodeValue);
            ...
            $imageUrl = $xpath->query('.//img', $item)->item(0)->getAttribute('src');
